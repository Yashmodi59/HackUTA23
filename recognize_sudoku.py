...
def recognize(image, model, old_sudoku):
    ...
    contours_img = cv2.drawContours(img_contours, contours, -1, (0, 255, 0), 3)
    ...
    if not (approx_90_degrees(angle_between(AB, AD), eps_angle) and approx_90_degrees(angle_between(AB, BC), eps_angle)
            and approx_90_degrees(angle_between(BC, DC), eps_angle) and approx_90_degrees(angle_between(AD, DC),
                                                                                          eps_angle)):
        return image
    ...
    # print(numbers)
    # imgDetectedDigits = displayNumbers(orginal_warp, numbers, color=(255, 0, 255))
    # show_image(imgDetectedDigits)
    ...
    sudoku_solver.solve(grid)  # Solve it
    if sudoku_solver.all_board_non_zero(grid):  # If we got a solution
